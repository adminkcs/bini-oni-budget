...
| 16 | v15 수정 |
| 17~19 | 초기 로딩 병목 계측 (임시) |
| 20 | 초기 로딩 최적화 — Sheets 왕복 축소 |
| 21 | 쓰기 경로 최적화 + 계측 제거 (현재 운영) |
...
**Apps Script의 비용은 데이터 양이 아니라 Sheets 백엔드 왕복 횟수에 비례한다.**
호출 1회당 약 200~500ms이고, 이것이 유일하게 의미 있는 최적화 대상이다.

실측 근거 (190행, JSON 23KB):

```
수입          2행  481ms      <- 2행짜리 시트도 0.5초
분류_설정    20행  456ms
예산_설정     4행  537ms
직렬화              1~2ms     <- 사실상 공짜
```

행 수와 소요 시간에 상관관계가 없다. 2행 시트와 190행 시트의 차이가 크지 않았다.

### 지켜야 할 규칙

| 하지 말 것 | 대신 |
|---|---|
| `getLastRow()` / `getLastColumn()` 를 값 읽기와 따로 호출 | `getDataRange().getValues()` 1회로 행수·열수·값을 모두 얻는다 |
| `getSheetByName()` 반복 호출 | `getSheets()` 1회 후 이름으로 매핑 |
| 빈 시트 확인용 추가 호출 | `values.length` 로 판별 |

> **실제로 저지른 실수:** 성능 개선이랍시고 빈 시트 가드로 `getLastRow`/`getLastColumn`을
> 넣었다가 시트마다 왕복을 2회씩 늘렸다. 왕복 약 15~20회 → 6회로 줄이자
> `doGet`이 **3.5~5.6초 → 2.46초**가 됐다.

### 남은 비용과 한계

체감 로딩 시간의 나머지 약 2.5~3.5초는 **Apps Script가 페이지를 중첩 iframe으로 감싸는
플랫폼 오버헤드**다. 코드로는 제거할 수 없다.

**캐시(CacheService)는 도입하지 않았다.** 서버 시간을 2.46초 → 0.5초로 줄일 수 있지만,
`onEdit`이 모든 시트 변경을 잡지 못해(스크립트·외부 API 변경, 트리거 오류) 최대 TTL만큼
오래된 데이터가 보일 수 있다. 가계부에서 틀린 숫자가 보이는 위험이 2초보다 크다고 판단했다.
나중에 데이터가 크게 늘어 다시 검토한다면, TTL을 짧게 잡고 모든 쓰기 경로에서
무효화하는 설계가 전제다.
